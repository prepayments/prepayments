...
export class CurrencyTableComponentsPage {
  ...
  deleteButtons = element.all(by.css('gha-currency-table div table .btn-danger'));
  title = element.all(by.css('gha-currency-table div h2#page-heading span')).first();
...
export class CurrencyTableUpdatePage {
  pageTitle = element(by.id('gha-currency-table-heading'));
...
export class CurrencyTableDeleteDialog {
  private dialogTitle = element(by.id('gha-delete-currencyTable-heading'));
  private confirmButton = element(by.id('gha-confirm-delete-currencyTable'));
